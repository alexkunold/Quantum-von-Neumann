...
#############################################
# EXAMPLE 3                                 #
# Linear atomic chain Hamiltonian           #
#                                           #
#############################################
...
def ham_coefs_dot(t, args):
    """Calculates the coefficients for the time
    derivative of the nuclear resonance Hamiltonian
    of a spin 1/2 particle.

    Output
    a_dot: numpy array containing the time derivative of
    ...
    dim, T, U = argsim, omega0, omega1, omega, phi = args
    a_dot = np.zeros(dim)
...
base = pauli_base(3)
sC, _ = structure_constants_recursive(3)
C = np.real(sC)
...
df_od.to_csv('Results/example-3/results_od.csv')
...
for tdiv in range(1, max_div + 1):
    ...
    df_qg.to_csv('Results/example-3/results_qg.csv')
